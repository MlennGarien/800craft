...
namespace fCraft.GUI {
    ...
    unsafe public sealed class IsoCat {
        ...
        private readonly Bitmap imageBmp;
        private readonly BitmapData imageData;
        private readonly int imageWidth, imageHeight;

        private readonly int dimX, dimY, dimX1, dimY1, dimX2, dimY2;
        private readonly int offsetX, offsetY;
        private readonly int isoOffset, isoX, isoY, isoH;
        private readonly int imageStride;
                        ...
                        fixed ( byte* stp = ShadowTiles ) {
                            ...
                            while ( z < Map.Height ) {
                                ...
                                if ( block != 0 ) {
                                    ...
                                    if ( x != ( Rot == 1 || Rot == 3 ? dimY1 : dimX1 ) )
                                        blockRight = GetBlock( x + 1, y, z );
                                    else
                                        blockRight = 0;
                                    if ( y != ( Rot == 1 || Rot == 3 ? dimX1 : dimY1 ) )
                                        blockLeft = GetBlock( x, y + 1, z );
                                    else
                                        blockLeft = 0;
                                    if ( z != Map.Height - 1 )
                                        blockUp = GetBlock( x, y, z + 1 );
                                    else
                                        blockUp = 0;
        ...
        private readonly int blendDivisor, mh34;
