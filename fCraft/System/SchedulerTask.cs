...
namespace fCraft {
    ...
    public sealed class SchedulerTask {
        ...
        public SchedulerTask RunRepeating( TimeSpan delay, TimeSpan interval, int times ) {
            if ( times < 1 )
                throw new ArgumentException( "Must be ran at least 1 time.", "times" );
            MaxRepeats = times;
            return RunForever( interval, delay );
        }
        ...
        #endregion Run Repeating

        #region Run Manual

        private static readonly TimeSpan CloseEnoughToForever = TimeSpan.FromDays( 36525 ); // >100 years
