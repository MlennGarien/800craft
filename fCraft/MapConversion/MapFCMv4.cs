...
namespace fCraft.MapConversion {
    ...
    public sealed class MapFCMv4 : IMapConverter {
        ...
        private const string ZoneMetaGroupName = "fCraft.Zones",
        ...
        private static Map LoadInternal( [NotNull] Stream stream, bool readLayers ) {
            if ( stream == null )
            ...
            if ( metaEntryCount < 0 )
                throw new MapFormatException( "Negative metadata entry count." );
            ...
            if ( layerCount < 0 )
                throw new MapFormatException( "Negative layer count." );
            ...
            if ( readLayers ) {
                for ( int l = 0; l < layerCount; l++ ) {
                    ...
                    if ( layerSize < 0 )
                        throw new MapFormatException( "Invalid layer size." );
        ...
        private static string ReadString( [NotNull] BinaryReader reader ) {
            ...
            if ( stringLength < 0 )
                throw new MapFormatException( "Negative string length." );
            return Encoding.ASCII.GetString( reader.ReadBytes( stringLength ) );
        }

        private static void WriteString( [NotNull] BinaryWriter writer, [NotNull] string str ) {
            if ( writer == null )
