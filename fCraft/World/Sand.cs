...
namespace fCraft
{
    public class Sand
    {
        ...
        public static void SandTrigger(Player player, int x, int y, int z, Block type)
        {
            if (type == Block.Sand )
            {
        ...
        static void Send(int x, int y, int z, int fh, Block type)
        {
        ...
        static void Send(int x, int y, int z, int fh, Block type, Player player)
        {
        ...
        static int Drop(int x, int y, int z)
        {
            if (z == 0)
                return 0;
            while (sandFall.Contains((Block)world.Map.GetBlock(x, y, z - 1)) && --z > 0) ; //WIN
            return z;
        }
    }
}