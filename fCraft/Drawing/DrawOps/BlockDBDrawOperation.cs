...
namespace fCraft.Drawing {
    ...
    public sealed class BlockDBDrawOperation : DrawOpWithBrush {
        ...
        private readonly int expectedMarks;
        ...
        public bool Prepare( Vector3I[] marks, BlockDBEntry[] changesToApply ) {
            if ( changesToApply == null )
                throw new ArgumentNullException( "changesToApply" );
        ...
        private BoundingBox FindBounds() {
            ...
            for ( int i = 0; i < changes.Length; i++ ) {
                if ( changes[i].X < min.X )
                    min.X = changes[i].X;
                if ( changes[i].Y < min.Y )
                    min.Y = changes[i].Y;
                if ( changes[i].Z < min.Z )
                    min.Z = changes[i].Z;
                if ( changes[i].X > max.X )
                    max.X = changes[i].X;
                if ( changes[i].Y > max.Y )
                    max.Y = changes[i].Y;
                if ( changes[i].Z > max.Z )
                    max.Z = changes[i].Z;
            }
